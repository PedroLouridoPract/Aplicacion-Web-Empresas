...
export default function AbsencesPage() {
  ...
  const filterBar = (vertical) => (
    <div className={vertical ? "flex flex-col gap-3" : "flex flex-wrap items-center gap-3"}>
      ...
      {!vertical && canCreate && (
        <button type="button" onClick={() => { setShowCreate(true); setError(""); }} className="ml-auto flex items-center gap-2 rounded-xl bg-indigo-400 px-6 py-3 text-sm font-semibold text-white shadow-sm transition-all not-disabled:hover:bg-indigo-500 not-disabled:hover:-translate-y-0.5 not-disabled:hover:shadow-[0_4px_12px_rgba(95,150,249,0.4)]">
          ...
          <div className="relative px-4 py-2">
            ...
            {selected.size > 0 && (
              <div className="absolute right-0 -top-1 flex items-center gap-2">
                <button type="button" disabled={bulkLoading} onClick={() => handleBulkAction("APPROVED")} className="rounded-xl bg-indigo-400 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition-all not-disabled:hover:bg-indigo-500 not-disabled:hover:-translate-y-0.5 not-disabled:hover:shadow-[0_4px_12px_rgba(95,150,249,0.4)] disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-indigo-300">Aceptar</button>
